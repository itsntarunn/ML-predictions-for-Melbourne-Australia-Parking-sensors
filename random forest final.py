...
def train_rf_model(X_train, y_train, X_val, y_val, X_test, y_test, datetime_test, device_ids_test, duration_minutes_test, scaler=None, n_estimators=100, max_depth=None):
    """
    Train a Random Forest model on the provided data and evaluate on the test set.
    ...
    y_test_prob = model.predict_proba(X_test_scaled)[:, 1]  # Probability estimates for the positive class
...
# Train and evaluate Random Forest model using the function
evaluation_metrics, result_data = train_rf_model(X_train.values, y_train.values,
                                                X_val.values, y_val.values,
                                                X_test.values, y_test,
                                                datetime_test, device_ids_test, duration_minutes_test)

# Save predictions and probabilities to Excel
predictions_file = r'C:\Users\langi longi\Desktop\New folder\rf_predictions.xlsx'
